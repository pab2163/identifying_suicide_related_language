...
iasr_talk_plot = maps_stb_or %>% 
  dplyr::filter(name_recode %in% c('True Positive', 'Flagged By Lexicon')) %>%
  mutate(name_recode = dplyr::recode(name_recode,
                                     'Flagged By Lexicon'= 'Flagged By Lexicon Alone',
                                     'True Positive'=' Suicide-Related (Human)',
                                     'Jokes/Hyperbole'=' Jokes/Hyperbole (Human)',
                                     'Current, 1st Person\n& Authentic'=' Current, 1st Person\n& Authentic STB (Human)')
         ) %>%
  ggplot(data = ., aes(x = name_recode, y = estimate, color = lex)) +
  geom_hline(yintercept = 1, lty = 2) +
  geom_point(size = 3) + 
...
ggsave(iasr_talk_plot, file = '../figures/talks/iasr_lexicon_human.png', 
       height = 3, width = 4)
```
...
# Daily negative binomial regression models
```{r}
maps_binary_days = maps %>%
  mutate(date = lubridate::date(tm_message_start)) %>%
  dplyr::select(ID, age, SSI19_i, Control, GROUP, date, all_of(binary_cols)) %>%
  mutate(SSI19_i_z = scale(SSI19_i),
         stb = ifelse(Control==1, 0, 1)) %>%
  pivot_longer(cols = all_of(binary_cols)) %>%
  group_by(ID, GROUP, Control, SSI19_i, age, date, name, SSI19_i_z, stb) %>%
  summarise(daily_count = sum(value),
            daily_total = n()) %>%
  ungroup() %>%
  group_by(ID) %>%
...
maps_binary_days_nest = maps_binary_days %>%
  group_by(name) %>%
  nest()
...
maps_si_z_models_days = maps_binary_days_nest %>%
  mutate(model = purrr::map(data, ~glmmTMB(data = ., daily_count ~ SSI19_i_z + age + offset(log(daily_total)) + total_entries + (1|ID), 
                                           family = nbinom2(link = 'log'))),
         coef = purrr::map(model, ~broom.mixed::tidy(., conf.int=TRUE, exponentiate=TRUE)))

maps_stb_models_days = maps_binary_days_nest %>%
  mutate(model = purrr::map(data, ~glmmTMB(data = ., daily_count ~ stb + age + offset(log(daily_total)) + total_entries + (1|ID), 
                                           family = nbinom2(link = 'log'))),
         coef = purrr::map(model, ~broom.mixed::tidy(., conf.int=TRUE, exponentiate=TRUE)))


maps_si_z_irr_days = maps_si_z_models_days %>% unnest(coef) %>%
  dplyr::select(-model, -data) %>%
  dplyr::filter(term == 'SSI19_i_z')

maps_stb_irr_days = maps_stb_models_days %>% unnest(coef) %>%
  dplyr::select(-model, -data) %>%
  dplyr::filter(term == 'stb')

coding_labels = read.csv('../data_stage2/coding_labels.csv')

maps_si_z_irr_all_categories_days = left_join(maps_si_z_irr_days, coding_labels, by = 'name')
maps_stb_irr_all_categories_days = left_join(maps_stb_irr_days, coding_labels, by = 'name')


maps_si_z_irr_all_categories_days = maps_si_z_irr_all_categories_days %>% mutate(
    panel = case_when(
...
maps_stb_irr_all_categories_days = maps_stb_irr_all_categories_days %>% mutate(
    panel = case_when(
      str_detect(name, "first") ~ "1st Person",
      str_detect(name, "second") ~ "2nd Person",
      str_detect(name, "third") ~ "3rd Person",
...
save(maps_stb_irr_all_categories_days, maps_si_z_irr_all_categories_days, file = '../model_outputs_step2/maps_coded_models_allcategories_nbinom.rda')
...
maps_stb_irr_all_categories_days = sig_star_label(maps_stb_irr_all_categories_days, 
                                            lower_col = conf.low,
                                            upper_col = conf.high,
                                            boundary = 1)

maps_si_z_irr_all_categories_days = sig_star_label(maps_si_z_irr_all_categories_days, 
                                            lower_col = conf.low,
                                            upper_col = conf.high,
                                            boundary = 1)

all_categories_stb_nbinom = maps_stb_irr_all_categories_days %>%
  dplyr::filter(! name %in% c('second_nssi', 'third_nssi', 'art')) %>%
  ggplot(data=., aes(x = label, y = estimate, color= panel)) +
  geom_hline(yintercept = 1, lty = 2) +
  geom_point(size = 2) + 
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0, lwd = 1) +
  geom_text(aes(label = sig_star, y = 0.05), size = 5, vjust = 0.3) +
  facet_grid(rows = vars(panel), scales = 'free_y') + 
  coord_flip() +
  scale_y_log10() +
  labs(y = 'Incident Rate Ratio\nLifetime STB > No STB History', x = NULL) +
  theme_bw() +
  theme(legend.position = 'none', text = element_text(face='bold'))

all_categories_si_nbinom = maps_si_z_irr_all_categories_days %>%
  dplyr::filter(! name %in% c('second_nssi', 'third_nssi', 'art')) %>%
  ggplot(data =., aes(x = label, y = estimate, color = panel)) +
  geom_hline(yintercept = 1, lty = 2) +
  geom_point(size = 2) + 
  geom_text(aes(label = sig_star, y = 0.1), size = 5, vjust = 0.3) +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0, lwd = 1) +
  facet_grid(rows = vars(panel), scales = 'free_y') + 
  coord_flip() +
  scale_y_log10() +
  labs(y = 'Incident Rate Ratio\nAssociation with Baseline SI', x = NULL) +
  theme_bw() +
  theme(legend.position = 'none', text = element_text(face='bold'))

maps_all_categories_stats_nbinom_plt = cowplot::plot_grid(all_categories_stb_nbinom, all_categories_si_nbinom,
                   labels = c('A', 'B'))

cowplot::save_plot(maps_all_categories_stats_nbinom_plt, 
                   base_height = 9, base_width = 9,
                   file = '../figures/supplemental/rev1_added/labels_stb_plot_all_categories_nbinom.png')



maps_stb_irr_all_categories_days %>% dplyr::select(name, estimate, conf.low, conf.high) %>% write.csv('../model_outputs_step2/maps_stb_history_irr_all_categories_nbinom.csv', row.names=FALSE)


maps_si_z_irr_all_categories_days %>% dplyr::select(name, estimate, conf.low, conf.high) %>% write.csv('../model_outputs_step2/maps_si_irr_all_categories_nbinom.csv', row.names=FALSE)
```
...
all_categories_stb = maps_stb_or_all_categories %>%
  ...
  coord_flip() +
  scale_y_log10() +
  labs(y = 'Odds Ratio: Lifetime STB > No STB History', x = NULL) +
  theme_bw() +
  theme(legend.position = 'none', text = element_text(face='bold'))
...
all_categories_si = maps_si_z_or_all_categories %>%
  ...
  coord_flip() +
  scale_y_log10() +
  labs(y = 'Odds Ratio: Association with Baseline SI', x = NULL) +
  theme_bw() +
  theme(legend.position = 'none', text = element_text(face='bold'))
...
cowplot::save_plot(maps_all_categories_stats_plt, 
                   base_height = 6, base_width = 9,
                   file = '../figures/main//labels_stb_plot_all_categories_short.png')
