...
removed_ids = dplyr::filter(final_spellcheck_codings, !id %in% maps_include_ids$study_id)

final_spellcheck_codings = dplyr::filter(final_spellcheck_codings, id %in% maps_include_ids$study_id)
```
...
flagged_df = final_spellcheck_codings %>%
  dplyr::filter(flag_final_no_emojis ==1) %>%
  dplyr::select(-`...1`, -id_coded, -filepath)

write.csv(flagged_df, file = '../data_stage2/maps_lexicon_flagged.csv', row.names=FALSE)
...
language_type_shorter = ggplot(cat_emmeans, aes(y = reorder_within(name, by = prob, within = panel), x = prob)) +
  geom_point(data = cat_emmeans,  aes(x = prob, y = reorder_within(name, by = prob, within = panel), color = panel)) +
  geom_errorbarh(data = cat_emmeans, 
                  aes(y = reorder_within(name, by = prob, within = panel), x = prob, xmin = asymp.LCL, xmax = asymp.UCL, color = panel)) +
  facet_grid(rows = vars(panel), scales = 'free_y', drop = TRUE, switch = 'both') +
...
ggsave(language_type_shorter, file = '../figures/supplemental/language_type_descriptive.png',
       height = 5, width = 6)
...
current_first_prop = true_positives %>%
  group_by(id) %>%
  summarise(n=n(), 
            prop_first = sum(current_first_person=='Yes')/n(),
            prop_notfirst = 1 - prop_first) %>%
  pivot_longer(contains('prop'),
               values_to = 'prop',
               names_to = 'current_first_person') %>%
  mutate(id = as.character(id),
         current_first_person = ifelse(current_first_person == 'prop_first', 'Yes', 'No')) %>%
  mutate(label = 'Individual Participants\n') 
...
cowplot::save_plot(content_grid, file = '../figures/main/rev1/human_labels.png', 
                   base_width = 6.8, base_height = 10)

cowplot::save_plot(content_grid, file = '../figures/main/rev1/human_labels.pdf', 
                   base_width = 6.8, base_height = 10)
