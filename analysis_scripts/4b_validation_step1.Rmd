---
title: 'Validate Youth Suicide Lexicon against Existing Approaches'
...
validation_step1_fig_washu = performance_metrics_summary_washu_F1 %>%
...
# validation figures
validation_step1_fig_maps = performance_metrics_summary_maps_F1 %>%
  dplyr::filter(Metric %in% c('F1', 'Recall', 'Precision')) %>%
  ggplot(aes(y = PredictionColumn, x = mean, color = is_youth_srl)) +
  geom_point() +
  geom_errorbarh(aes(xmin = lwr95, xmax = upr95), height = 0.1) +
...
validation_plot = cowplot::plot_grid(validation_step1_fig_maps, validation_step1_fig_washu,
                                     nrow = 2, align = 'v',
...
# Does spellcheck change performance in the MAPS data? 
...
# Plot comparison with versus without spellcheck
