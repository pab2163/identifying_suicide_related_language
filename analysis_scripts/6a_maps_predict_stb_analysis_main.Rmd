...
maps_entries_per_participant = maps %>%
...
predict_entries_per_participant = predict %>%
  mutate(date = as.Date(tm_message_start)) %>%
  group_by(ID, lifetime_stb, SSI19_total_i) %>%
  summarise(n_entries = n()) %>%
  mutate(cohort = 'PREDICT') %>%
  dplyr::filter(!(is.na(lifetime_stb) & is.na(SSI19_total_i)))

```

# Tally included versus excluded in MAPS and PREDICT

```{r}
predict_entries_per_participant = mutate(predict_entries_per_participant, inc_1000 = n_entries >= 1000)
maps_entries_per_participant = mutate(maps_entries_per_participant, inc_1000 = n_entries >= 1000)


maps_entries_per_participant %>%
  group_by(inc_1000) %>%
  count()

predict_entries_per_participant %>%
  group_by(inc_1000) %>%
  count()


maps_entries_per_participant %>%
  group_by(inc_1000, lifetime_stb) %>%
  count()
...
dplyr::filter(by_id_summary_maps, total_flagged > 0) %>% nrow() / nrow(by_id_summary_maps)
...
maps_excluded = dplyr::filter(maps, total_message < 1000)
...
## Before primary analyses, remove participants with under 1000 total messages
...
m_maps_si_days <- glmmTMB(
  any_daily_flagged ~ SSI19_i + age + daily_count + total_message + (1 | ID),
  family = binomial(link = "logit"),
  data = maps_days %>% na.omit() %>% mutate(SSI19_i = scale(SSI19_i))
)

m_maps_si_days_pct <- glmmTMB(
  daily_flag_pct ~ SSI19_i + age + daily_count + total_message + (1 | ID),
...
maps %>% group_by(ID, Control) %>% count() %>%
  group_by(Control) %>%
  count()
...
m_maps_stb_days <- glmmTMB(
  any_daily_flagged ~ Control + age + daily_count + total_message + (1 | ID),
  family = binomial(link = "logit"),
  data = maps_days %>% na.omit()
)

m_maps_stb_days_pct <- glmmTMB(
  daily_flag_pct ~ Control + age + daily_count + total_message + (1 | ID),
  data = maps_days %>% na.omit()
)
...
write.csv(timeofday_or_maps, file = '../model_outputs_step2/maps_time_of_day_rev1.csv', row.names=FALSE)
...
table(predict_demo$lifetime_stb)
...
predict_demo %>%
  ...
  mutate(lifetime_stb = ifelse(
    ...
    stb_group = case_when(
      lifetime_stb ==1 ~ 'Lifetime STB',
      lifetime_stb ==0 & (group=='MDD' | group=='r emMDD') ~ 'Lifetime MDD',
      lifetime_stb ==0 & group=='HC' ~ 'HC',
    )
  ) %>%
  dplyr::select(-contains('cssrs')) %>%
...
m_predict_si_days <- glmmTMB(
  any_daily_flagged ~ SSI19_total_i + age + daily_count + total_message + (1 | ID),
  family = binomial(link = "logit"),
...
sum(is.na(predict$ID))
sum(is.na(predict$age))
sum(is.na(predict$lifetime_stb))
sum(is.na(predict$flag_bin))
...
m_predict_si_schoolclosure <- glmmTMB(
  flag_bin ~ SSI19_i + age + schoolclosure + (1 | ID),
  family = binomial(link = "logit"),
  data = dplyr::select(predict, ID, age, flag_bin, SSI19_total_i, schoolclosure) %>% mutate(SSI19_i = scale(SSI19_total_i)) %>% na.omit() 
)
...
cowplot::save_plot(main_panel, filename = '../figures/main/rev1/fig2_maps_predict_stb_history.png',
...
write.csv(timeofday_or_combined, file = '../model_outputs_step2/maps_predict_time_of_day_rev1.csv', row.names=FALSE)

```

## Models for raw quantity of text entries by time of day
...
predict_entries_per_day = predict %>%
  mutate(date = as.Date(tm_message_start)) %>%
...
m_maps_entries_per_day = lmerTest::lmer(data = maps_entries_per_day, n_entries ~ Control + (1|ID))
m_predict_entries_per_day = lmerTest::lmer(data = predict_entries_per_day, n_entries ~ lifetime_stb + (1|ID))

summary(m_maps_entries_per_day)
summary(m_predict_entries_per_day)


broom.mixed::tidy(m_maps_entries_per_day, conf.int=TRUE)
broom.mixed::tidy(m_predict_entries_per_day, conf.int=TRUE)


stb_n_entries_emmeans = plyr::rbind.fill(m_maps_entries_per_day %>% emmeans::emmeans(specs = ~Control) %>%
  data.frame() %>%
  mutate(cohort = 'MAPS', 
         lifetime_stb = ifelse(Control ==1, 0, 1)), 
  m_predict_entries_per_day %>% emmeans::emmeans(specs = ~lifetime_stb) %>%
  data.frame() %>%
  mutate(cohort = 'PREDICT'))
...
## Make MAPS + PREDICT combined time of day plot
```{r}
...
cowplot::save_plot(time_social_app_plt, file = '../figures/main/rev1/time_social_app_grid.png',
                   base_height = 7, base_width = 9.15)
```

# Proportions of translated entries / entries at night
