...
one_bootstrap = function(seed, df){
  ...
  si_z_human = glmmTMB(
    ...
    family = binomial(link = "logit"),
    data = dplyr::select(boot_sample, ID, age, SSI19_i, suicide_related) %>% na.omit() %>% mutate(SSI19_i = scale(SSI19_i)))
  ...
  stb_z_human = glmmTMB(
    ...
    family = binomial(link = "logit"),
    data = dplyr::select(boot_sample, ID, age, lifetime_stb, suicide_related) %>% na.omit())
...
for (i in 1:n_iter){
  ...
  save(boot_output, time_elapsed, file = '../model_outputs_step2/bootstrap_outputs_maps_human_vs_lexicon.rda')
}
